    ...
    if request.method == 'POST':
        if 'rtngbtn' in request.POST:
            ...
            if result['1'] > result['-1']:
                ...
                res = 'negative'
            rev_instance = Ratings.objects.create(user=request.user,product=productdetails,ratings=rati,reviews=review,sentiments=res)
...
def topproducts(request):
    prod=Ratings.objects.all().values('product__name').annotate(rati=Avg('ratings')).order_by('-rati')
